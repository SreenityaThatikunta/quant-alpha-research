...
def test_target_can_model_a_delayed_entry_without_using_signal_day_returns():
    dates = pd.date_range("2024-01-01", periods=8, freq="B")
    benchmark = pd.DataFrame({"date": dates, "open": np.arange(100, 108), "close": np.arange(100, 108)})
    panel = pd.DataFrame({"date": dates, "ticker": "ABC", "open": np.arange(50, 58), "close": np.arange(50, 58)})
    result = add_residual_return_target(panel, benchmark, horizon_days=2, beta_lookback_days=2, entry_delay_days=2)
    expected = panel.loc[3, "close"] / panel.loc[2, "open"] - 1
    assert np.isclose(result.loc[0, "stock_forward_return"], expected)
    assert result["stock_forward_return"].tail(3).isna().all()
