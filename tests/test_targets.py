...
def test_target_is_strictly_forward_and_final_rows_are_missing():
    ...
    benchmark = pd.DataFrame({"date": dates, "close": np.arange(100, 108)})
    panel = pd.DataFrame({"date": dates, "ticker": "ABC", "close": np.arange(50, 58)})
    result = add_residual_return_target(panel, benchmark, horizon_days=2, beta_lookback_days=2)
    expected_stock_forward = panel.loc[2, "close"] / panel.loc[0, "close"] - 1
